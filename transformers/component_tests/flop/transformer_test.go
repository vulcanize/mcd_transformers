...
var _ = Describe("Executing the flop transformer", func() {
	...
	It("reads in a vat storage diff and persists it", func() {
		...
		diff := utils.StorageDiff{
			KeccakOfContractAddress: transformer.HashedAddress,
			BlockHash:               blockHash,
	...
	Describe("bids", func() {
		...
		Describe("guy + tic + end packed slot", func() {
			...
			BeforeEach(func() {
				_, writeErr := db.Exec(flop.InsertFlopKicksQuery, blockNumber, blockHash.Hex(), strings.ToLower(flopperContractAddress), bidId)
