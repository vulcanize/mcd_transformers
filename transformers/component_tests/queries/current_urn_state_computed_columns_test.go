...
var _ = Describe("current_urn_state computed columns", func() {
	var (
		db               *postgres.DB
		fakeBlock        int
		fakeGuy          = "fakeAddress"
	...
	Describe("current_urn_state_ilk", func() {
		It("returns the ilk for an urn", func() {
			...
			getIlkErr := db.Get(&result,
				`SELECT ilk_identifier, rate, art, spot, line, dust, chop, lump, flip, rho, duty, pip, mat, created, updated
					FROM api.current_urn_state_ilk(
					(SELECT (urn_identifier, ilk_identifier, ink, art, created, updated)::api.current_urn_state
					FROM api.current_urn_state))`)
	...
	Describe("current_urn_state_frobs", func() {
		It("returns frobs for an urn_state", func() {
			...
			getFrobsErr := db.Get(&actualFrobs,
				`SELECT ilk_identifier, urn_identifier, dink, dart FROM api.current_urn_state_frobs(
                        (SELECT (urn_identifier, ilk_identifier, ink, art, created, updated)::api.current_urn_state
                         FROM api.current_urn_state))`)
		...
		Describe("result pagination", func() {
			...
			It("limits results to latest block number if max_results argument is provided", func() {
				...
				getFrobsErr := db.Select(&actualFrobs,
					`SELECT ilk_identifier, urn_identifier, dink, dart FROM api.current_urn_state_frobs(
						(SELECT (urn_identifier, ilk_identifier, ink, art, created, updated)::api.current_urn_state
						 FROM api.current_urn_state), $1)`, maxResults)
				Expect(getFrobsErr).NotTo(HaveOccurred())
			...
			It("offsets results if offset is provided", func() {
				...
				getFrobsErr := db.Select(&actualFrobs,
					`SELECT ilk_identifier, urn_identifier, dink, dart FROM api.current_urn_state_frobs(
						(SELECT (urn_identifier, ilk_identifier, ink, art, created, updated)::api.current_urn_state
						 FROM api.current_urn_state), $1, $2)`, maxResults, resultOffset)
				Expect(getFrobsErr).NotTo(HaveOccurred())
	...
	Describe("current_urn_state_bites", func() {
		It("returns bites for an urn_state", func() {
			...
			getBitesErr := db.Get(&actualBites, `
				SELECT ilk_identifier, urn_identifier, ink, art, tab FROM api.current_urn_state_bites(
				    (SELECT (urn_identifier, ilk_identifier, ink, art, created, updated)::api.current_urn_state
				    FROM api.current_urn_state))`)
